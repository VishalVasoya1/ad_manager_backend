"""Auth router Ã¢â‚¬â€ login and logout endpoints."""
...
from app.router.admin.auth.auth_access import AuthQuery
from app.router.admin.auth.auth_validator import AuthLoginRequest, AuthResponse
    ...
    async def login(self, request: Request, body: AuthLoginRequest, db: AsyncSession = Depends(get_db)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                module="auth",
                action="login",
                description=f"Admin '{user.email}' (role: {user.role}) logged in successfully from {ip_address or 'unknown IP'}.",
                ip_address=ip_address,
            )
    ...
    async def logout(self, request: Request, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                module="auth",
                action="logout",
                description=f"Admin (ID: {user_id}) logged out and token was blacklisted from {ip_address or 'unknown IP'}.",
                ip_address=ip_address,
            )
