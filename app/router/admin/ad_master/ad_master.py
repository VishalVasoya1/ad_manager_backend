"""Ad Master router Ã¢â‚¬â€ CRUD and search. Write operations are admin-only."""
...
from app.router.admin.ad_master.ad_master_validator import AdMasterCreateRequest, AdMasterUpdateRequest, AdMasterOut, AdMasterSearchOut
...
class AdMasterRouter:
    ...
    def __init__(self):
        self.router = APIRouter(prefix="/admin/ads/v1/admaster", tags=["Ad Master"])
    ...
    async def create(self, request: Request, body: AdMasterCreateRequest, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                action="create",
                reference_id=ad.id,
                description=f"Created ad master entry '{ad.title}' of type '{ad.type}'.",
    ...
    async def get_all(self, request: Request, user_id: Optional[UUID] = Query(None), type: Optional[str] = Query(None), db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                module="ad_master",
                action="view",
                description=f"Listed {len(items)} ad master entries with {filter_str}.",
                ip_address=ip_address,
            )
            await db.commit()
    ...
    async def get(self, request: Request, ad_master_id: UUID, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        endpoint = f"/admaster/{ad_master_id}"
        try:
            admin_id = UUID(payload["user_id"])
            ip_address = request.client.host if request.client else None
            ...
            await ActivityService.log_activity(
                ...
                description=f"Viewed ad master entry '{ad.title}' (type: {ad.type}).",
                ip_address=ip_address,
            )
            await db.commit()
    ...
    async def delete(self, request: Request, ad_master_id: UUID, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                action="delete",
                reference_id=ad_master_id,
                description=f"Deleted ad master entry '{ad.title}' (type: {ad.type}).",
                ip_address=ip_address,
            )
            await db.commit()
