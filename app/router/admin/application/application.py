"""Application router Ã¢â‚¬â€ CRUD and API key reset. Write operations are admin-only."""
...
class ApplicationRouter:
    ...
    def __init__(self):
        ...
        self.router.post("/{app_id}/resetapikey", status_code=status.HTTP_200_OK, dependencies=[Depends(require_admin)])(self.reset_api_key)
    ...
    async def create(self, request: Request, body: ApplicationCreateRequest, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                action="create",
                reference_id=app.id,
                description=f"Created '{app.name}' ({app.type}) application with package '{app.package_name or 'N/A'}', status '{app.status}', assigned to user '{body.assign_by}'.",
    ...
    async def get_all(
        ...
        try:
            ...
            if app_type: filters.append(f"type='{app_type}'")
            ...
            await ActivityService.log_activity(
                ...
                module="application",
                action="view",
                description=f"Listed applications (page {page}, size {size}) with {filter_str}. Total results: {total}.",
                ip_address=ip_address,
            )
            await db.commit()
    ...
    async def get(self, request: Request, app_id: UUID, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                action="view",
                reference_id=app_id,
                description=f"Viewed application '{app.name}' (type: {app.type}, status: {app.status}, package: {app.package_name or 'N/A'}).",
                ip_address=ip_address,
            )
            await db.commit()
    ...
    async def delete(self, request: Request, app_id: UUID, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        try:
            ...
            await ActivityService.log_activity(
                ...
                action="delete",
                reference_id=app_id,
                description=f"Deleted application '{app.name}' (type: {app.type}, package: {app.package_name or 'N/A'}).",
                ip_address=ip_address,
            )
            await db.commit()
    ...
    async def reset_api_key(self, request: Request, app_id: UUID, db: AsyncSession = Depends(get_db), payload=Depends(jwt_bearer)):
        ...
        endpoint = f"/application/{app_id}/resetapikey"
        try:
            admin_id = UUID(payload["user_id"])
            ip_address = request.client.host if request.client else None
            app = await ApplicationQuery.get_by_id(app_id, db)
